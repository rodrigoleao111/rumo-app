...
| `SurfaceWhite` | `#FFFDF9` | — | Branco quente — superfície de cards (sobe acima do fundo areia) |
...
background       = Sand            // fundo da Activity (areia)
...
Borda `CardBorder` (GreenMoss 31% alpha) presente em todos os cards — substitui a sombra como delimitador de superfície. Bordas são preferidas a sombras porque preservam o visual limpo no fundo `Sand`.
...
Button(colors = ButtonDefaults.buttonColors(containerColor = AmberPrimary)) {
    Text("Próximo →", color = GreenMoss)
}
```
`AmberPrimary` sólido com texto `GreenMoss` (escuro) — exclusivo para botões de progressão no wizard. Diferencia visualmente "avançar no fluxo" de "confirmar uma ação".

> **Regra de contraste do âmbar:** conteúdo sobre `AmberPrimary` é sempre **escuro** (`GreenMoss`), nunca branco. O âmbar `#E9B43C` é claro demais para texto/ícone branco (contraste ~1.8:1, reprova AA). Vale para FAB, snackbar, botões âmbar e badge de contagem regressiva. Espelha a referência (chips "em X dias", botão "Uber").
...
Não usa `NavigationBar` do Material 3. Implementação customizada, alinhada ao board de identidade (barra clara, item ativo em pill verde) — **somente ícone, sem rótulo**:

```
Row (SurfaceWhite, RoundedCornerShape(32dp), shadow verde suave 16dp)
 └─ PillNavItem × N
      └─ Box (52×36dp, raio 14dp)
           ├─ fundo GreenMoss se selecionado, senão transparente
           └─ Icon (Color.White se ativo, GreenMoss 45% alpha se inativo, 22dp)
```

A barra é clara (`SurfaceWhite`) sobre o fundo creme, com sombra verde de baixo alpha (0.12–0.20) para o efeito flutuante. O item ativo é um **pill verde sólido com ícone branco**; os inativos são ícones em verde apagado. A barra fica flutuante com `padding` lateral de 16dp, separada da tela.
...
Fundo `AmberPrimary` com conteúdo `GreenMoss` (escuro) em todas as snackbars do app. Sem botão de ação. Duração padrão (`SHORT`). Exibida após operações de escrita bem-sucedidas ("Alterações salvas ✓").
...
- Usar `SurfaceWhite` como fundo de card, `Sand` como fundo de tela
- Bordas de card: `BorderStroke(1.dp, CardBorder)` — nunca omitir em fundo `Sand`
...
- Snackbar: `containerColor = AmberPrimary, contentColor = GreenMoss`
- Conteúdo sobre âmbar (FAB, snackbar, botões, badge de contagem): sempre `GreenMoss` (escuro), nunca branco
